...
// register Blog_Sidebar_Widget_Author_Bio widget
function blog_sidebar_widget_register_author_bio_widget() {
    register_widget( 'Blog_Sidebar_Widget_Author_Bio' );
}
add_action( 'widgets_init', 'blog_sidebar_widget_register_author_bio_widget' );

function blog_sidebar_widget_author_load_sortable() {    
    wp_enqueue_script( 'jquery-ui-core' );    
    wp_enqueue_script( 'jquery-ui-sortable' );    
}
add_action( 'load-widgets.php', 'blog_sidebar_widget_author_load_sortable' );

if( ! class_exists( 'Blog_Sidebar_Widget_Author_Bio' ) ) : 
 /**
 * Adds Blog_Sidebar_Widget_Author_Bio widget.
 */
class Blog_Sidebar_Widget_Author_Bio extends WP_Widget {

	/**
	 * Register widget with WordPress.
	 */
	function __construct() {
        add_action( 'admin_print_footer_scripts', array( $this,'blog_sidebar_widget_socicon_template' ) );
		parent::__construct(
			'blog_sidebar_widget_author_bio', // Base ID
			__( 'Blog Sidebar: Author Bio', 'blog-sidebar-widget' ), // Name
			array( 'description' => __( 'An Author Bio Widget', 'blog-sidebar-widget' ), ) // Args
		);
	}
    ...
    * @since 1.0.0
    */
    function blog_sidebar_widget_socicon_template() { 
    	$screen = get_current_screen();?>
	        <div class="blog-sidebar-widget-socicon-template">
	            <li class="social-share-list" data-id="{{socicon_index}}">
                    <span class="blog-sidebar-widget-social-icons-sortable-handle"></span>
                    <span class="blog-sidebar-widget-social-icons-field-handle"><i class="fas fa-plus"></i></span>
                    <label for="<?php echo esc_attr( $this->get_field_id( 'socicon_profile' ) ); ?>"><?php esc_html_e( 'Social Icon', 'blog-sidebar-widget' ); ?></label>
                    <span class="example-text">Example: facebook</span>
                    <input class="user-social-profile" id="<?php echo esc_attr( $this->get_field_id( 'socicon_profile[{{socicon_index}}]' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'socicon_profile[{{socicon_index}}]' ) ); ?>" type="text" value="" placeholder="<?php _e('Search Social Icons','blog-sidebar-widget');?>" />
                    <label class="link-label" for="<?php echo esc_attr( $this->get_field_id( 'socicon[{{socicon_index}}]' ) ); ?>"><?php esc_html_e( 'Link', 'blog-sidebar-widget' ); ?></label>
	        ...
	        <style type="text/css">.blog-sidebar-widget-socicon-template{display: none;}</style>
    <?php
    }
	...
	public function widget( $args, $instance ) {
        ...
        if ( is_active_widget( false, false, $this->id_base, true ) ) {
            ...
            wp_enqueue_style( 'Dancing-Script', BSW_FILE_URL .'/public/css/dancing-script.min.css', array(), '1.0.0', 'all' );
        }
        ...
        if ( !filter_var( $image, FILTER_VALIDATE_URL ) === false ) {
            $attachment_id = $obj->blog_sidebar_widget_get_attachment_id( $image );
        }

        if ( !filter_var( $author_image, FILTER_VALIDATE_URL ) === false ) {
            $author_image_id = $obj->blog_sidebar_widget_get_attachment_id( $author_image );
        }        
        ...
        ?>
        <div class="blog-sidebar-widget-author-bio-holder">
                        ...
                        foreach ( $arr_keys as $key => $value )
                        {
                            if ( array_key_exists( $value, $instance['socicon'] ) )
                            {
                                ...
                                {       
                                    ...
                                    {
                                        $icon = $this->blog_sidebar_widget_get_social_icon_name( $instance['socicon'][$value] );
        ...
        <?php    
        $html = ob_get_clean();
        echo apply_filters( 'blogsidebar_author_bio_widget_filter', $html, $args, $instance );
    ...
    public function form( $instance ) {
        ...
        jQuery(document).ready(function($){
            $(".blog-sidebar-widget-sortable-icons").sortable({
                    cursor: "move",
                    update: function (event, ui) {
                        $(".blog-sidebar-widget-sortable-icons .social-share-list input").trigger("change");
                    }
                });
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>"><?php esc_html_e( 'Author Name', 'blog-sidebar-widget' ); ?></label> 
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'name' ) ); ?>" type="text" value="<?php echo esc_attr( $name ); ?>" />
        </p>
        
        <p>
            <label><?php _e('Display photo from:','blog-sidebar-widget'); ?></label>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'author-image-option' . '-gravatar' ) );?>" class="radio-btn-wrap"><?php _e('Gravatar', 'blog-sidebar-widget');?></label>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'author-image-option' . '-photo' ) );?>" class="radio-btn-wrap"><?php _e('Uploaded Photo','blog-sidebar-widget');?></label>
        </p>
        
        <?php $obj->blog_sidebar_widget_get_image_field( $this->get_field_id( 'author-image' ), $this->get_field_name( 'author-image' ), $author_image, __( 'Upload Author Image', 'blog-sidebar-widget' ) ); ?>
        
        <p class="author-email">
            <label for="<?php echo esc_attr( $this->get_field_id( 'email' ) ); ?>"><?php esc_html_e( 'Author Email', 'blog-sidebar-widget' ); ?></label> 
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'email' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'email' ) ); ?>" type="text" value="<?php echo esc_attr( $email ); ?>" />
        </p>
        <div class="widget-side-note" class="example-text"><?php $link1 = '<a href="http://en.gravatar.com/" target="_blank">Gravatar</a>'; echo sprintf( __( 'You can show your %1$s image instead of manually uploading your photo. Just add your gravatar registered email address here.','blog-sidebar-widget'), $link1 );?></div>
        
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'content' ) ); ?>"><?php esc_html_e( 'Description', 'blog-sidebar-widget' ); ?></label>
        ...
        <p>
            <label><?php _e('Display Signature from:','blog-sidebar-widget'); ?></label>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'author-signature-option' . '-text' ) );?>" class="radio-btn-wrap"><?php _e('Text','blog-sidebar-widget');?></label>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'author-signature-option' . '-photo' ) );?>" class="radio-btn-wrap"><?php _e('Uploaded Photo','blog-sidebar-widget');?></label>
        </p>

        <?php $obj->blog_sidebar_widget_get_image_field( $this->get_field_id( 'image' ), $this->get_field_name( 'image' ), $image, __( 'Upload Signature Image', 'blog-sidebar-widget' ) ); ?>
        ...
        <p class="signature-text">
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'signature-text' ) ); ?>"><?php esc_html_e( 'Signature Text', 'blog-sidebar-widget' ); ?></label>
		</p>

        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'label' ) ); ?>"><?php esc_html_e( 'Button Label', 'blog-sidebar-widget' ); ?></label> 
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'link' ) ); ?>"><?php esc_html_e( 'Button Link', 'blog-sidebar-widget' ); ?></label>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'target' ) ); ?>"><?php esc_html_e( 'Open in New Tab', 'blog-sidebar-widget' ); ?></label>
        </p>

        <ul class="blog-sidebar-widget-sortable-icons" id="<?php echo esc_attr( $this->get_field_id( 'blog-sidebar-widget-social-icons' ) ); ?>">
		<?php
                    ...
                    { 
                        if(isset( $instance['socicon'][$value] ) && !empty( $instance['socicon'][$value] ))
                        {
        		            ...
        		            <li class="social-share-list" data-id="<?php echo $value;?>">
        		                <span class="blog-sidebar-widget-social-icons-sortable-handle"></span>
        		                <span class="blog-sidebar-widget-social-icons-field-handle"><i class="<?php echo esc_attr( $class );?>"></i></span>
                                <label for="<?php echo esc_attr( $this->get_field_name( 'socicon_profile['.$value.']' ) ); ?>"><?php esc_html_e( 'Social Icon', 'blog-sidebar-widget' ); ?></label>
                                <span class="example-text">Example: facebook</span>
                                ...
                                <label class="link-label" for="<?php echo esc_attr( $this->get_field_id( 'socicon['.$value.']' ) ); ?>"><?php esc_html_e( 'Link', 'blog-sidebar-widget' ); ?></label>
                                <span class="example-text">Example: http://facebook.com</span>
		...
		?>
        <span class="blog-sidebar-widget-socicon-holder"></span>
        </ul>
		<input type="button" name="button" id="add-user-socicon" class="button button-primary" value="<?php _e('Add Social Profile','blog-sidebar-widget');?>"><br>
...
} // class Blog_Sidebar_Widget_Author_Bio
endif;